...
# Input: 
#   userIDs: vector containing unique user IDs
#   itemIDs: vector containing unique item IDs
...
def sample_implicit_negatives(user_ids, item_ids, num_items_to_sample, df_exclude,  p = None):
    ...
    user_exclude_items = [ (user, np.array(pd.DataFrame(df_small).item, dtype=int)) for (user, df_small) in gpd]
